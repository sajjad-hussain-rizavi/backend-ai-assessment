...
(function () {
  ...
  document.querySelectorAll('.sym-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (prev !== symbol) {
        fetch('/api/ask/symbol-change', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ symbol: prev }),
        }).catch(() => {});
      }
      fetchTicker();
    });
  ...
  function replaceThinking(el, answer, meta, marketCtx) {
    if (!el) return;
  ...
  async function send() {
    ...
    // Abort if the server takes more than 55 s (just under Railway's 60 s limit)
    ...
    try {
      ...
      // Always parse JSON; surface HTTP errors as readable messages
      let data;
      try { data = await resp.json(); }
      catch { throw new Error('Server returned an unreadable response (HTTP ' + resp.status + ')'); }

      if (!resp.ok || data.error) {
        thinkEl.remove();
        addSys('Error: ' + (data.error || 'HTTP ' + resp.status), true);
      } else if (!data.answer) {
        thinkEl.remove();
        addSys('Error: received an empty answer from the AI. Please try again.', true);
      } else {
        replaceThinking(thinkEl, data.answer, { durationMs: data.durationMs, model: data.model }, data.marketContext);
      ...
      const msg = err.name === 'AbortError'
        ? 'Request timed out after 55 s. The server may be overloaded.'
