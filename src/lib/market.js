 ...
 * Results are cached for 30 s — repeated calls within that window are free.
...
async function getPrice(symbol = config.binance.symbol) {
  ...
  return cache.get(key, async () => {
    ...
    try {
      ...
      log.info({ duration: Date.now() - t0, price: data.price }, 'Binance price fetched');
      return { symbol: data.symbol, price: data.price, fetchedAt: new Date().toISOString() };
    } catch (err) {
      log.error({ duration: Date.now() - t0, err: err.message, status: err.response?.status }, 'Failed to fetch Binance price');
      throw normaliseError(err, 'Could not retrieve current price');
    }
  });
}
 ...
 * Results are cached for 30 s per unique symbol+interval+limit combination.
 * @param {Object} opts
...
async function getKlines({ symbol = config.binance.symbol, interval = '1h', limit = 24 } = {}) {
  ...
  const key = `klines:${symbol}:${interval}:${safeLimit}`;

  return cache.get(key, async () => {
    const log = logger.child({ fn: 'getKlines', symbol, interval, limit: safeLimit });
    const t0  = Date.now();
    try {
      const { data } = await client.get('/api/v3/klines', {
        params: { symbol, interval, limit: safeLimit },
      });
      log.info({ duration: Date.now() - t0, count: data.length }, 'Binance klines fetched');
      return {
        symbol,
        interval,
        limit: safeLimit,
      ...
      log.error({ duration: Date.now() - t0, err: err.message, status: err.response?.status }, 'Failed to fetch Binance klines');
      throw normaliseError(err, 'Could not retrieve klines');
    }
  });
}
